    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                for (int j = 0; j < _FilterKernelSectors; j++)
                {
                    float sigmaSq = abs(standardDeviations[j].r + standardDeviations[j].g + standardDeviations[j].b);
                    float alpha = 1 / (1 + pow(255 * sigmaSq, 0.5f * _FilterSharpness));

                    sumAlpha += alpha;
                    sumAlphaWeights += alpha * weightedAverages[j];
                }
